...
namespace FMODUnityResonance
{
    ...
    public static class FmodResonanceAudio
    {
        ...
        private static readonly List<FmodResonanceAudioRoom> enabledRooms = new();
        ...
        private static DSP ListenerPlugin
        {
            get
            {
                if (!listenerPlugin.hasHandle()) listenerPlugin = Initialize();
        ...
        public static void UpdateAudioRoom(FmodResonanceAudioRoom room, bool roomEnabled)
        {
            ...
            if (roomEnabled)
            {
                if (!enabledRooms.Contains(room)) enabledRooms.Add(room);
            }
            else
            {
                ...
                for (var currentBus = 0; currentBus < numBusses; ++currentBus)
                {
                    ...
                    if (channelGroup.hasHandle())
                    {
                        ...
                        for (var currentDsp = 0; currentDsp < numDsps; ++currentDsp)
                        {
                            ...
                            if (dspNameSb.Equals(listenerPluginName) && dsp.hasHandle()) return dsp;
                        }
                    }
                }
            }
